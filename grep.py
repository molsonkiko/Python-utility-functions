...
def process_grep_query(query,has_warned_fname = False):
	try: #make the query
	...
	if any(('p' in x) for x in options): #used -p option, open files
		...
		if opening_decision =='y':
			...
			for filename in resultset:
				if isinstance(filename,tuple): #'-m' option used
					filename = filename[0]
...
def main():
	...
	if len(args) > 1: #first arg is always path to the module's file
		query = ' '.join(args[1:])
		has_warned_fname = process_grep_query(query,has_warned_fname)
		return
	print(helpstring)
	while True:
		#break
